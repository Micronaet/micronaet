...
class account_invoice(osv.osv):
    ...
    # on change function:
    def onchange_extra_address(self, cr, uid, ids, extra_address, partner_id, context = None):
    ...
    _columns = {
        'extra_address': fields.selection([
            ('none', 'None'),
            ('contact', 'Contact'),
            ('partner', 'Partner'),], 'Extra address', select = True, readonly = False, required = True),
        'partner_ids': fields.many2many('res.partner', 'invoice_partner_rel', 'invoice_id', 'partner_id', 'Extra partner'),
    }
