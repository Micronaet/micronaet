...
class product_product_extra_fields(osv.osv):
    ...
    def schedule_etl_product_import(self, cr, uid, path, file_name, context=None):
        ...
        _logger = logging.getLogger('base_micronaet')
                       ...
                       try:
                           ...
                           if item: # update
                              try:
                                  ...
                                  _logger.error("Error modify product [%s]"%(sys.exc_info()[0],))
                               ...
                               try:
                                  ...
                                  _logger.info('Create Product ref: %s'%(ref,))
                                   ...
                                   _logger.error("Error create product [%s]"%(sys.exc_info()[0],))

                       except:
                           # KO log:
                           _logger.info('Error importing line: %s [%s]'%(line,sys.exc_info()[0]))
                           continue
                       ...
                       try:                          
                           if len(line): # jump empty lines
                               if tot_colonne == len(line): # tot # of colums must be equal to # of column in first line
                                   ...
                                   hour_cost_1 = PrepareFloat(line[csv_id])            
                                   csv_id+=1
                                   hour_cost_2 = PrepareFloat(line[csv_id])            
                                   hour_cost = hour_cost_1 or hour_cost_2
                                       ...
                                       if item: # update
                                          ...
                                          try:
                                              ...
                                              try: # Riprovo a scrivere rimuovendo la vat
                                                  ...
                                                  _logger.error('Error update partner ref: %s [%s]'%(ref,sys.exc_info()[0]))  
                                                  counter['err_upd']+=1  
                                       else: # new
                                          counter['new'] += 1  
                                          try:
                                              ...
                                              try: # Riprovo a scrivere rimuovendo la vat
                                                  ...
                                                  _logger.error('Error create partner ref: %s [%s]'%(ref,sys.exc_info()[0]))  
                                                  counter['err']+= 1  
                                   else: # destination
                                   ...
                                   if not partner_id:  
                                      _logger.error('Error no partner ID ref: %s [%s]'%(ref,sys.exc_info()[0]))  
                           ...
                           # KO log:
                           _logger.error('Error importing line: %s [%s]'%(line,sys.exc_info()[0]))
                           continue
                                    
                _logger.info('End sync of partner type: %s \nCounter:%s'%(mexal_type, counter))
        except:
            _logger.error('Error scheduler operation during: %s [%s]'%(comment,sys.exc_info()[0]))                    
