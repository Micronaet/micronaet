...
def CreateTitle(sock,dbname,uid,pwd,titles,table):
    ...
    for title in titles:
        title_id = sock.execute(
            dbname, uid, pwd, 'res.partner.title', 'search', [('name', '=', title)])
        if not len(title_id):
           title_id=sock.execute(
...
class product_product_extra_fields(osv.osv):
    ...
    def schedule_etl_product_import(self, cr, uid, context=None):
            ...
            try:
                ...
                data = {
                    ...
                    'statistic_category': "%s%s" % (
                        ...
                        "%02d" % int(
                            line['NKY_CAT_STAT_ART'] or '0') if
                        line['CKY_CAT_STAT_ART'] else '',)

                    # 'procure_method': 'make_to_order', 'purchase_ok': True,
                    # 'categ_id': category_id, 'chemical_category_id':
                    # chemical_category_id,
                    # 'need_analysis': True, 'partner_ref': sale_name,
                    # 'mexal_c': ref,  # TODO utilizzato ref per la
                    # sincronizzazione!
                }
                ...
                if item:  # update
                    ...
                    try:
                        data['supply_method'] = supply_method
                        # produce, buy (setup only during creation)
                ...
                if len(ref) > 6:  # Is a child product?
                    ...
                    if father in father_dbp_ids:
                        father_dbp_ids[father].append(product_id)
                        # create list for child
                    else:
                        father_dbp_ids[father] = [product_id]
                        # create list for child
        ...
        for item in force_update_uom:
            try:
                self.force_uom(
                    cr, uid, item, force_update_uom[item], context=context)
            except:
                _logger.error(
                    _("Error updating UOM for product!"))
        ...
        for element in father_dbp_ids:
            ...
            if supply_method:
                self.write(cr, uid, father_dbp_ids[element], {
        ...
        cursor = self.pool.get('micronaet.accounting').get_product_level(
            cr, uid, store=1, context=context)
        if not cursor:
        ...
        if error_list:
            _logger.info('Code import errors: [%s]' % (error_list, ))
            # todo  Log in a dashboard
        return True
