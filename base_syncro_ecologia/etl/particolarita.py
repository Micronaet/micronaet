...
def GetPricelist(sock, dbname, uid, pwd, customer_id, model_pricelist): # 2 returned values in dict
    ...
    # Rilevo i dati utente (per associare poi il listino e per il nome
    partner_ids = sock.execute(dbname, uid, pwd, 'res.partner', 'search', [('mexal_c', '=', customer_id)]) 
    partner_item = sock.execute(dbname, uid, pwd, 'res.partner', 'read', partner_ids) 
    
    if not partner_item:
       print "[ERR] Partner non trovato:" + customer_id # TODO controllare meglio se fare tutto quello che viene dopo qui
       
    partner_name=partner_item[0]['name'] or ''   
    name="Listino cliente: %s"%(partner_name)
    pl_id = sock.execute(dbname, uid, pwd, 'product.pricelist', 'search', [('mexal_id', '=', customer_id)]) 
    if pl_id: 
       pricelist_id= pl_id[0]  
       pricelist_mod=sock.execute(dbname,uid,pwd,'product.pricelist','write',pl_id, {'name':name,
    ...
    version="Versione cliente: %s"%(partner_name)
    version_id = sock.execute(dbname, uid, pwd, 'product.pricelist.version', 'search', [('mexal_id', '=', customer_id)])
    if version_id: # modify
       pricelist_version_id= version_id[0] 
       pricelist_version_mod=sock.execute(dbname,uid,pwd,'product.pricelist.version','write',version_id, {'name':version,
