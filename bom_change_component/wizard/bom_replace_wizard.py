...
class MrpBomReplaceWizard(orm.TransientModel):
    """ Wizard for change component
    """
    _name = 'mrp.bom.replace.wizard'
    ...
    def action_run_in_context_mode(self, cr, uid, ids, context=None):
        ...
        # Pool used:
        bom_line_pool = self.pool.get('mrp.bom')
        ...
        bom_line_ids = bom_line_pool.search(cr, uid, [
            ('bom_id', '!=', False), # Component bom only (not parent)
        ...
        if mode == 'excel':
            ...
            return excel_pool.return_attachment(
                cr, uid, 'Esito modifica', version='7.0', php=True,
                context=context)
