...
class Parser(report_sxw.rml_parse):
    ...
    def get_objects(self, objects=None, wizard_data=None):
        ''' Generate loop elements:
            if comes from openerp.report return objects
        ...
        pickin_pool = self.pool.get('stock.picking')
        line_ids = pickin_pool.search(
            self.cr, self.uid, data_filter_domain, order="name") #via_number")
        return pickin_pool.browse(self.cr, self.uid, line_ids)
