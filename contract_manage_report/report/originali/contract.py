...
class Parser(report_sxw.rml_parse):
    ...
    def get_totals_account(self, type_of, account_id):
        ...
        #elif type_of == 'general':    
        #    return t_general.get(account_id, 0.0)
        elif type_of == 'balance':
            res = t_invoice.get(account_id, 0.0) + t_cost.get(account_id, 0.0) + t_hour_cost.get(account_id, 0.0) #+ t_invoice.get(account_id, 0.0) * () 
    ...
    def get_totals(self, field, subtotalize=True, general_amount = 0.0):
        ...
        if field == 'general':
            res = general_amount # 0.0 - totals.get('invoice', 0.0) * general_rate
        else:    
    ...
    def filter_description(self, data = None, short = False):
        ...
        if short:
            ...
            res += "  [ %s - %s - %s - %s - %s ]"%("interventi visibili" if data.get('intervent', True) else "interventi non visibili", 
    ...
    def get_intervent(self, account_id, data = None):
        ...
        global totals, t_hour, t_hour_cost, t_operation, t_general
