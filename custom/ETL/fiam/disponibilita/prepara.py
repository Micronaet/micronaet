...
try:
        ...
        if len(line): 
           ...
           if codice[0] in ["A", "B", "D", "E", "F", "G", "I", "K", "L", "M", "N", "O", "P", "R", "S", "T", "V", "W", "Z"]:
              ...
              if not scartato: # Sommo eventuale magazzino Fiam e scrivo il file
                 ...
                 tot+=1
                 s = codice + ";" + descrizione.replace("Ø", "diam.").replace("ø", "diam.") + ";" + dispo.replace(",0","").replace(".0","") + ";" + arrivi.replace(",0","").replace(".0","") + ";" + prenotati.replace(",0","").replace(".0","") + ";"  + prezzo + "\n"
                 fout.write(s)
    ...
    for codice in dispo_fiam.keys():
        if codice not in dispo_gpb: # Se il codice non è presente nei codici per GPB:
           ...
           arrivi=str(arrivi).replace(".",",")   
           tot+=1 
           s = codice + " (F);" + descrizione + ";" + dispo.replace(",0","").replace(".0","") + ";" + arrivi.replace(",0","").replace(".0","") + ";0;" + dispo_fiam[codice][3] + "\n" #(F) = codice presente solo in Fiam
           fout.write(s)
    fout.close()
