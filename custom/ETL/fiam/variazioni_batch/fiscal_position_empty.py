...
partner_ids = sock.execute(dbname, uid, pwd, 'res.partner', 'search', ['|', ('type_cei', 'not in', ('e','c','i')), ('property_account_position', '=', False)])
partner_read = sock.execute(dbname, uid, pwd, 'res.partner', 'read', partner_ids, ('id', 'name', 'country', 'property_account_position', 'type_cei'))
...
for partner in partner_read:
    ...
    #pdb.set_trace()
    print "Modifico: Partner: %s, Nazione: %s, CEI: %s, Pos. F.: %s >> CEI: %s, Pos F. %s"%(partner['name'], partner['country'] and partner['country'][1], partner['type_cei'], partner['property_account_position'], type_CEI, fiscal_position)
