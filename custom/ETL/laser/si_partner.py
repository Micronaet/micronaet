...
# Modules ETL "Servizi alle imprese"
...
actual_category="Servizi alle imprese"
...
try:
    for line in lines:
        if len(line): # jump empty lines
            ...
            name = line[1 + start].title()
            piva = "IT" + line[2 + start].zfill(13)       # filled with left 0 TODO check country code
            dimension = line[3 + start].title()    # M2O 
            sector = line[4 + start]               # M2O
            localization = line[5 + start].title() # Provincia # TODO manage "province"
            # Country?
            ...
            if item:  # test if record exists (basing on Ref. as code of Partner)
               ...
               data={'import': ref,
                     ...
                     'domain': domain,
                     'vatnumber': piva, 
                     'title_dimension_id': CreateDimension(dbname,uid,pwd,dimension),
                     'title_sector_id': CreateSector(dbname,uid,pwd,sector),
                     'customer': True,
                     'office_id': office_id,
                     #'category_id': [(6,0,[category_id])], # m2m
                     'comment': localization, # TODO create list of "province" / "regioni"
