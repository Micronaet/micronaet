...
config.read([os.path.expanduser('~/ETL/panchemicals/openerp.cfg')])#['openerp.cfg']) # if file is in home dir add also: , os.path.expanduser('~/panchemicals/openerp.cfg')])
...
# FUNCTION:
def get_agent_id(sock, dbname, uid, pwd, out_file, agent_code, name_agente):
    ''' get agend ID (user)
    '''
    if agent_code:
        item_ids=sock.execute(dbname, uid, pwd, 'res.users', 'search', [('mexal_id', '=', agent_code)])
        if item_ids:
           return item_ids[0]
        else:
           raise_error("Agente/Responsabile/Supervisore non trovato negli utenti di OpenERP: %s - %s"%(agent_code,name_agente), out_file, "E")  # solo se non trovato (non per i vuoti)
    return False
        ...
        if counter['tot']<0:  # jump n lines of header 
           ...
           if len(line): # jump empty lines
               if tot_colonne == len(line): # tot # of colums must be equal to # of column in first line
                   ...
                   csv_id+=1
                   mexal_resp = Prepare(line[csv_id])                      # codice reponsabile 
                   csv_id+=1
                   descrizione_responsabile = Prepare(line[csv_id])        # descrizione responsabile

                   mexal_super = "460.00500" # Vago                        # Codice supervisore
                   
                   # Calcolo i codici utente per indicare agente, responsabile, superuser (segnalare eventuali utenti da creare!
                   user_id = get_agent_id(sock, dbname, uid, pwd, out_file, mexal_resp, "Responsabile [%s] %s - Ditta: %s"%(mexal_resp, descrizione_responsabile, name))   # NOTE: Responsabile = user_id                   
                   supervisor_id = get_agent_id(sock, dbname, uid, pwd, out_file, mexal_super, "Supervisore per cliente [%s] %s"%(mexal_super, name))
                   ...
                   if not mexal_destination: # create partner only with c or s
                       data={'name': name,
                             ...
                             'mexal_super': mexal_super,
                             'mexal_resp': mexal_super,
                             'mexal_age': ref_agente,
                       ...
                       if mexal_type=='c': # and not destination!
                          ...
                          # Inserimento informazioni agente / responsabile / supervisore: (solo per i clienti)
                          data['user_id']=user_id # TODO agent only for customer?
                          data['mexal_resp']=mexal_resp              # codice responsabile (= agente in OpenERP)
                          
                          data['supervisor_id']=supervisor_id        # cerco utente responsabile abbinato a user_id  # TODO agent only for customer?
                          data['mexal_super']=mexal_super            # codice responsabile in mexal
                          
                          data['mexal_age']= ref_agente              # codice agente esterno 
                          data['mexal_age_description']= name_agente # Nome agente esterno
