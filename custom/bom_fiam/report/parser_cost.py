...
class Parser(report_sxw.rml_parse):
    ...
    def get_objects_product(self):
        ...
        active_ids=self.pool.get('mrp.bom').search(self.cr, self.uid, [('bom_id', '=', False)], order='name')
    ...
    def get_value(self, bom_id, type_of="min"):
        ...
        for component in components.bom_lines:             # loop on component
            no_component=""
            value=0                
            for seller in component.product_id.seller_ids: # loop on seller
                for pricelist in seller.pricelist_ids:          # loop on pricelist
                    if pricelist.price > 0 and pricelist.is_active: # only positive active price                       
                       if type_of=="min":   
                          if not value: # set up first min
                             value=pricelist.price
                          if pricelist.price < value: # compare values
                             value=pricelist.price
                       else: # suppose max
                          if pricelist.price > value: # compare values
                             value=pricelist.price
