...
class res_partner_address(osv.osv):
    ...
    def on_change_zip(self, cr, uid, ids, zipcode):
        res = {'value':{}}
        if(zipcode):
            city_id = self.pool.get('res.city').search(cr, uid, [('zip', '=', zipcode)])
            if city_id:
                city_obj = self.pool.get('res.city').browse(cr, uid, city_id[0])
                res = {'value': {
                    'province':city_obj.province_id.id,
...
class res_partner_contact(osv.osv):
    ...
    def on_change_city_contact(self, cr, uid, ids, trainee_location):
        try:
            if(trainee_location):            
                res = self.pool.get('res.partner.address').on_change_city(cr, uid, ids, trainee_location)
                res['value']['trainee_location']=res['value']['city']
                del res['value']['city']  
                del res['value']['country_id']
            return res
        except:
            return {'value':{}} # if errore empty value returned

    def on_change_zip_contact(self, cr, uid, ids, zipcode):
        try:
            if(zipcode):            
                res = self.pool.get('res.partner.address').on_change_zip(cr, uid, ids, zipcode)
                res['value']['trainee_location']=res['value']['city']
                del res['value']['city']  
                del res['value']['country_id']
            return res
