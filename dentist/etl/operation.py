...
if len(sys.argv) != 3 :
   print """
         *** Syntax Error! ***
         *  Use the command with this syntax: python ./operation.py F_SCHEOP.CSV F_PREVEN.CSV
         *********************
         """ 
   sys.exit()
...
operation_esit = sock.execute(dbname, uid, pwd, 'dentist.operation', 'unlink', operation_ids)
        ...
        if counter['tot']<0:  # jump n lines of header 
           ...
           if len(line): # jump empty lines
               if tot_colonne == len(line): # tot colums equal to column first line
                   ...
                   csv_id+=1 # State 
                   state = prepare(line[csv_id]).upper() # E A N 
                   csv_id+=1 # Quotation date
                   date = prepare_date(line[csv_id]) or False
                   csv_id+=1 # Pricelist
                   pricelist = prepare_date(line[csv_id]) or False
                   # Importo            # Sconto               # Codmed
                   # Fp                 # Tipomov              # Unico
                   # Periodo            # Evidenzia            # Costope
                   # Saldope            # Plurincas            # Datainc
                   ...
                   product_data = {
                                  ...
                                  'date': date,
                                  'date_end': date,
                                  'state': 'done', # TODO solo se è E #'draft' 'done' 'cancel'
                                  'import': True,
                                  }
                   
                   # OPERATION CREATION ***************
                   try:
                       item_id = sock.execute(dbname, uid, pwd, 'dentist.operation', 'create', product_data) 
                   except:
...
# TRIGGER operazioni:
esito=sock.execute(dbname, uid, pwd, 'dentist.operation', 'xml_rpc_operation_complete')
