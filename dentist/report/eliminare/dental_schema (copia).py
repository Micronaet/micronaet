...
def get_array_list(self, item_id, side, is_order):        
    ...
    # list used for loop all teeth if there's all side (upper or lower):
    all_side_list= {'u': [str(item) for item in range(11,19)] + [str(item) for item in range(21,29)],
                    'd': [str(item) for item in range(31,39)] + [str(item) for item in range(41,49)],
                   }
    ...
    for tooth in tooth_list_ids:
      if tooth.tooth: # TODO raise the jump if no teeth!
        if (side.upper()=="U" and (tooth.tooth[:1] in ("*", "u", "1", "2", "5", "6",))) or (side.upper()=="D" and (tooth.tooth[:1] in ("d", "3", "4", "7", "8",))): 
           if tooth.tooth[:1] in ("u", "d"): # Note "u" has always side ="U" and "d" has always side="D"
              tooth_range_list = all_side_list[tooth.tooth[:1]]
           else:
              tooth_range_list=(tooth.tooth,)
           
           for item_tooth in tooth_range_list:   
               pos = get_id(item_tooth)
               level[pos] += 1
               ...
               if item_tooth[:1] in ("5", "6", "7", "8"):
                  code_tooth = tooth.product_id.code.lower() + "*" if is_order else "%s*\n%s"%(tooth.product_id.code.lower(), tooth_date)
               else:
                  code_tooth = tooth.product_id.code.upper() if is_order else "%s\n%s"%(tooth.product_id.code.upper(), tooth_date)
