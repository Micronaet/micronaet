...
<%
dim WorkFolder, workFolderUp, WorkFolderRoot, TipoApertura
TipoApertura=Request.querystring("apertura")
select case TipoApertura
       ...
       case "3": ' aperto per navigazione
	        WorkFolder=Request.querystring("attuale")
			WorkFolderUp=Request.querystring("precedente") 
			WorkFolderRoot=getCartellaUpload ()         
end select	   
...
' Codice utente per operazioni caricamento dati videata
   Dim fso, f, f1, fc, s, fold1, foldN
   Set fso = CreateObject("Scripting.FileSystemObject")
   Set f = fso.GetFolder(WorkFolder)
   Set fc = f.Files   
   Set foldN=f.SubFolders
%>
...
<style type="text/css">
<!--
@import url(/docnaet/stili/documenti.css);
-->
</style>
...
<form name="frmElenco">
  <table width="100%" border="0">
    <tr> 
      <td colspan="6"> 
        <div align="center"><b>
		<% if TipoApertura="2" then ' cartella utente %>
	...
	<% 
	dim i, colore
	i=0 ' azzero qui per tutto
    ' ******************************** caricamento delle eventuali cartelle ****************************
	if TipoApertura="3" then 
	' rappresento la cartella per tornare indietro di livello (solo se non sono nella root
	   if WorkFolder<>WorkFolderRoot then
	   %>
	      <tr> 
		  <td width="5%" bgcolor=""#FFFFCC"" > 
			<div align="center"> 
			<input type="hidden" name="chkSelezione" value="<%=WorkFolderUp%>">
            <a href="<%=WorkFolderUp%>"><img src="../../../../immagini/sistema/bottoni/icone/esplora.gif" width="17" height="17" border="0"></a></div>
		  </td>
		  <td width="43%" bgcolor="<%=colore%>">Cartella precedente</td>
		  <td width="52%" bgcolor="<%=colore%>"></td>
		</tr>
		<%
	   end if
	 end if
	 ' rappresento tutte le eventuali cartelle presenti nella cartella selezionata
	 For Each fold1 in foldN
			i=i+1 
		...
		<tr> 
		  ...
		  <div align="center"> 
		 		<a href="upload1.asp?apertura='2',attuale=<% =fold1.ShortName %>"><img src="../../../../immagini/sistema/bottoni/icone/esplora.gif" width="17" height="17" border="0"></a></div>
		  </td>
		  <td width="43%" bgcolor="<%=colore%>"><a href="#" target="_blank"><%=fold1.Name%></a></td>
    ...
    %>
    <% 
	' ********************************* caricamento degli eventuali files ******************************
	 For Each f1 in fc
	    i=i+1 
		if i mod 2=0 then colore="#CEECFD" else colore="#FFFFCC" 
	%>
