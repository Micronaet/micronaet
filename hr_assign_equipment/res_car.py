...
class res_car(osv.osv):
    ...
    _columns = {
        ...
        'typology':fields.selection([
                                    ...
                                    ('PC','PC'),
                                    ('generic','Generic'),
                                ],'Tipology', select=True, readonly=False, required=True),
        'plate': fields.char("Plate ID or S/N", size=256 , required=True, help="Identification numbero for equipment, plate for car, S/N for portable or PC etc"), 
        ...
        'telephone_company': fields.char("Telephone company", size=50),
        #'telephone_note': fields.text('Note'), # TODO togliere le note fare un hide delle etichette semplicemente
                                
        # PC: 
        'pc_ref': fields.char("Ref.", size=256),
        'model': fields.char("Model", size=50),
        #'pc_note': fields.text('Note'),

        # Generic: 
        'generic_ref': fields.char("Ref.", size=256),
        #'generic_note': fields.text('Note'),
    }
...
class res_car_service(osv.osv):
    ...
    _columns = {
        ...
        'required_next_service': fields.boolean('Have next service ?'),
        'note':fields.text("Note"),
        'spent': fields.float('Spent', required=True),
    }
    ...
    def _check_dates(self, cr, uid, ids, context=None):
        ...
        for i in self.read(cr, uid, ids, ['id','employee_id','car_id', 'start_date','end_date'], context=context):
            ...
            cr.execute( "select * from res_car_contract where state in ('draft', 'assigned') "\
                        ...
                        " )", (i.get('id',0),car_id[0],date_start,date_start,date_start,date_end)
                       ) # tolto emp_id[0], e employee_id = %s or prima di car_id (l'impiegato può avere più oggetti)
    ...
    def action_assigned(self, cr, uid, ids):
        for contract in self.read(cr,uid,ids):
            if contract['start_date'] == time.strftime('%Y-%m-%d'):
                ...
                self.pool.get('hr.employee').write(cr,uid,[contract['employee_id'][0]], {'car_id':contract['car_id'][0]}) # TODO mettere lista di cose attive
            else:
