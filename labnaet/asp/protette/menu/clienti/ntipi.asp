...
If (CStr(Request(MM_flag)) <> "") Then
  MM_dupKeyRedirect="terrore.asp"
  MM_rsKeyConnection=MM_SQLDocnaet_STRING
  MM_dupKeyUsernameValue = CStr(Request.Form("txtDescrizione"))
  MM_dupKeySQL="SELECT tipDescrizione FROM Tipi WHERE tipDescrizione='" & MM_dupKeyUsernameValue & "'"
...
function validateForm(Oggetto) { //v4.0
    if (!Oggetto.value) 
	{
	   alert ('Inserire necessariamente il nome del tipo di ditta!');
	   return false;
    }
	else 
	{
...
<form name="frmInserimento" method="POST" action="<%=MM_editAction%>">
    ...
    <table width="28%" border="0" bgcolor="#CCCCCC">
      <tr>
        <td align="center"> <b><font color="#009933">Modifica Tipo</font></b></td>
    </tr>
  </table> </div> <br>
  <table width="28%" border="0" align="center">
    <tr> 
      <td width="23%" bgcolor="#FFFFCC">Tipo:</td>
