<%@LANGUAGE="VBSCRIPT"%>
<!--#include file="Connections/SQLDocnaet.asp" -->
<!--#include file="librerie/codice/gestione.asp" -->
<%
if Request ("Remoto")=1 THEN 
     Session("Remoto")=TRUE 
	 Application("CartellaDati")=Application("CartellaDatiR")
else 
     Session("Remoto")=FALSE
	 Application("CartellaDati")=Application("CartellaDatiN")
end if	 
set Ditte = Server.CreateObject("ADODB.Recordset")
Ditte.ActiveConnection = MM_SQLDocnaet_STRING
dim MultiAzienda
if GestisceMultiAzienda then 
   MulitAzienda = "" 
else 
   MultiAzienda="WHERE ditLicenziataria='1' " ' faccio vedere solo la azienda licenziataria
end if

Ditte.Source = "SELECT *  FROM Ditte "+MultiAzienda+"  ORDER BY ditLicenziataria DESC, ditRagioneSociale ASC"
Ditte.CursorType = 0
...
<table width="100%" border="0" align="center" height="100%" bgcolor="#FFFFFF" bordercolor="#FFFFFF" cellpadding="0" cellspacing="0">
  <tr bgcolor="#FFFFFF"> 
    <td align="center" height="120" bgcolor="#CEECFD" width="2%"><b><font color="#990000"> 
      <br>
      </font></b></td>
    <td align="center" height="120" width="98%"><b><font color="#990000"><img src="immagini/sistema/bottoni/icone/<%if Session("Remoto") then Response.Write("portatile.gif") else Response.Write("rete.gif")%>" width="31" height="31"><br>
      <%if Session("Remoto") then Response.Write("Remote logon") else Response.Write("Net logon") %>
      <br>
      </font></b></td>
  </tr>
...
%>
          ...
          <tr> 
            ...
            <td align="center" bgcolor="#CEECFD" width="105" > 
              <input type="checkbox" name="chkRidotto" value="1" <%if VersioneRidotta(False) then Response.Write("checked")%>>
              Ridotto </td>
  ...
  <tr bgcolor="#999999"> 
    ...
    <td height="60" width="98%" bgcolor="#FFFFFF">&nbsp;</td>
  </tr>
</table>
</body>
</html>
