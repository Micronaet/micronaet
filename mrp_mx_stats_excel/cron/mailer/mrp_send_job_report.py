...
# Mail:
smtp = {
    'text': '''<p>Production Details dated: {}</p><b>Micronaet S.r.l.</b>'''.format(now),
    'subject': 'PCA Production Details : {}'.format(now),
...
fullname = mrp.server_action_extract_excel_job_report_saveas()
...
for to in smtp['to']:
    ...
    msg['To'] = ','.join(smtp['to']) # XXX See all delivery!
    msg.attach(MIMEText(smtp['text'], 'html'))

    part = MIMEBase('application', 'octet-stream')
    part.set_payload(open(fullname, 'rb').read())
    Encoders.encode_base64(part)
    part.add_header('Content-Disposition', 'attachment; filename="%s"' % os.path.basename(fullname))
