...
class MWS(object):
    ...
    # All subclasses must define their own URI only if needed or different
    URI = "/"
    ...
    #def __init__(self, access_key, secret_key, account_id, domain='https://mws.amazonservices.com', uri="", version=""):
    def __init__(self, access_key, secret_key, account_id, domain='https://mws-eu.amazonservices.com', uri="", version=""):   # TODO parametrize for different market address!
    ...
    def validate_xml_with_xsd(self, xml_text, xsd_text):
        ''' Validate XML text passed with XSD text passed
            return bolean response 
        '''
        from lxml import etree
        from StringIO import StringIO
        
        xmlschema = etree.XMLSchema(etree.parse(StringIO(xsd_text)))
        print xmlschema.validate(etree.parse(StringIO(xml_text)))
    ...
    def make_request(self, extra_data, method="GET", **kwargs):
        ...
        headers = {'User-Agent': 'openerp-amazon-mws/0.0.1 (Language=Python)'}
        headers.update(kwargs.get('extra_headers', {}))

        try:
            # I don't pass the params dict as the params argument to request
            # because here i have to get the url parsed string of params in order to sign it, so
    ...
    def enumerate_param(self, param, values):
        """
            From param, values: 
                ('MarketplaceIdList.Id', (123, 345, 4343))
            Returns
            {
...
class Feeds(MWS):
    """ Amazon MWS Feeds API 
        ...
        FBA Shipment Injection             _POST_FULFILLMENT_ORDER_REQUEST_DATA_           XML
        Fulfillment Feed 
        FBA Shipment Injection             _POST_FULFILLMENT_ORDER_CANCELLATION            XML
        ...
        Flat File Order                    _POST_FLAT_FILE_ORDER_ACKNOWLEDGEMENT_DATA_     Tab delimited
        Acknowledgement Feed 
        
        Flat File Order Fulfillment Feed   _POST_FLAT_FILE_FULFILLMENT_DATA_
        Tab delimited
        
        Flat File FBA Shipment             _POST_FLAT_FILE_FULFILLMENT_ORDER_REQUEST_DATA_ Tab delimited
        Injection Fulfillment Feed 
        
        Flat File FBA Shipment             _POST_FLAT_FILE_FULFILLMENT_ORDER_CANCELLATION  Tab delimited
        Injection Cancellation Feed        _REQUEST_DATA_
        
        FBA Flat File Create Inbound       _POST_FLAT_FILE_FBA_CREATE_INBOUND_SHIPMENT_    Tab delimited
        Shipment Feed 

        FBA Flat File Update Inbound       _POST_FLAT_FILE_FBA_UPDATE_INBOUND_SHIPMENT_    Tab delimited
        Shipment Feed
        
        Flat File Order Adjustment         _POST_FLAT_FILE_PAYMENT_ADJUSTMENT_DATA_        Tab delimited
        Feed 
        
        Flat File Invoice Confirmation     _POST_FLAT_FILE_INVOICE_CONFIRMATION_DATA_      Tab delimited
        Feed 
        ...
        Flat File Video Loader File        _POST_FLAT_FILE_LISTINGS_DATA_                  Tab delimited
        Flat File Price and Quantity       _POST_FLAT_FILE_PRICEANDQUANTITYONLY            Tab delimited
        Update File                        _UPDATE_DATA_ 
        Product Ads Flat File Feed         _POST_FLAT_FILE_SHOPZILLA_DATA_                 Tab delimited
        UIEE Inventory File                _POST_UIEE_BOOKLOADER_DATA_                     Universal Information
                                                                                           Exchange Environment (UIEE)
    """
    ACCOUNT_TYPE = "Merchant"
