...
class sale_order_line(osv.osv):
    ...
    _columns = {
        ...
        'price_comment': fields.text(
            "Price comment",
            help="Extra comment about price used in this quotation")
    }
...
class res_partner_pricelist_product(osv.osv):
    ...
    def _get_price_from_pricelist(
        ...
        if partner_id:
            ...
            pricelist['pricelist1'] = \
                ...
                partner_proxy.property_product_pricelist else False
            pricelist['pricelist2'] = \
                partner_proxy.pricelist_model_id.id if \
                partner_proxy.pricelist_model_id else False
            pricelist['pricelist3'] = \
                partner_proxy.pricelist_model_history_id.id if \
                partner_proxy.pricelist_model_history_id else False
            ...
            for field in pricelist:
                if pricelist[field]:
                    res[item.id][field] = pricelist_pool.price_get(
                        cr, uid, [pricelist[field]], item.product_id.id, 1.0,
                        False, {
                            'uom': False,    # uom
    ...
    _columns = {
        'in_pricelist': fields.boolean(
            'A listino', help='Incluso nella stampa di listino cliente'),
        'pricelist1': fields.function(
            _get_price_from_pricelist, method = True, type = 'float',
            string='Prefinito', help='Prezzo di listino predefinito',
            store=False, multi="Pricelist"),
        'pricelist2': fields.function(
            _get_price_from_pricelist, method=True, type='float',
            string='Modello', store=False, multi="Pricelist"),
        'pricelist3': fields.function(
            _get_price_from_pricelist, method=True, type='float',
            string='Storico', store=False, multi="Pricelist"),
        }
...
class res_partner(osv.osv):
    ...
    def schedules_pricelist_partner(self, cr, uid, file_name, context=None):
        ...
        # for problems in pricelist starting with cost price = 0
        bug_start_value = 0.0
        tot = 0
                    ...
                    if len(line):  # jump empty lines
                        ...
                        if accounting_id not in pricelist:
                            ...
                            if partner_ids:
                                ...
                                pricelist[accounting_id] = [
                                    ...
                                    False,                      # 2. pl
                                    False,                      # 3. plv
                                    partner_proxy.pricelist_model_history_id.id
                                    if partner_proxy.pricelist_model_history_id
                                    else False,  # 4. history (for reference for extra product)
                                ]
                            else:
                                _logger.error(
                                    "[Row %s] Partner not found: %s" % (
                                        tot, accounting_id))
                                continue
                        ...
                        if not pricelist[accounting_id][2]:
                            ...
                            if pricelist_ids:
                                ...
                                self.write(
                                    cr, uid, pricelist[accounting_id][0],
                        ...
                        # test pricelist version presence
                        if not pricelist[accounting_id][3]:
                        ...
                        if price: # if exist price prepare PL item
                            item_data = {
                                ...
                                # base price (product.price.type) todo:
                                'base': 1,
                                'min_quantity': 1,
                                # Recharge on used base price:
                                'price_surcharge': price - bug_start_value,
                                'product_id': product_openerp_id,
                                }
                            ...
                            if item_ids:  # update
                                item_pool.write(
                                    cr, uid, item_ids, item_data,
                                    context=context)
                            else:
                                item_pool.create(
                                    cr, uid, item_data, context=context)
                        _logger.info("[Row %s] PL item import %s:" % (
                            tot, line))
            except:
