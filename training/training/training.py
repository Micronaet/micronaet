...
class training_course(osv.osv):
    ...
    _columns = {
        'splitted_by' : fields.selection([('1', '1 Hours'),
                                          ('2', '2 Hours'),
                                          ('3', '3 Ore'),
                                          ('4', '4 Hours'),
                                          ('8', '8 Hours')
...
class training_session(osv.osv):
    ...
    def _participant_count(self, cr, uid, ids, fieldnames, args, context=None):
        res = dict.fromkeys(ids, 0)
        if ids: # capitava che entra anche con ids pari a [] (in questo caso non si vedono i totali)
           # TODO: Se non ci sono partecipanti da errore nel chiudere il calendario
    ...
    def _check_date_of_seances(self, cr, uid, ids, context=None):
        for session in self.browse(cr, uid, ids, context=context):
            for seance in session.seance_ids:
                if seance.date[:10] < session.date[:10]: # Confronto solo per data
                    return False
    ...
    def _create_seance(self, cr, uid, session, context=None):
        ''' 
           Crea un calendario partendo dalla data della edizione (anche in draft) e 
           riportando fino alla fine la settimana tipo, saltando domeniche e festivita'
           
           Il programma interpreta se l'edizione viene ricavata utilizzando FET o la settimana tipo
           Cosi' facendo nel primo caso riporta pari pari i dati, nel recondo la replica per tutta 
           la durata dell'edizione
           
           return res (lista degli ID delle lezioni create, verranno linkate 
                       dal chiamante all'edizione)
        ''' 
        ...
        seance_proxy = self.pool.get('training.seance')
        holiday_proxy = self.pool.get('training.holiday.year')  # elenco anni creati
        holiday_period_proxy = self.pool.get('training.holiday.period') # elenco periodi holiday creati
        week_proxy = self.pool.get('training.seance.plan') # Piano di lezioni settimanale
        group_proxy = self.pool.get('training.group')

        # Verifiche pre partenza:
        #  BLOCCANTI #######################################################################################
        #   1. Date edizione correttamente impostate (inizio e fine obbligatorie)
        loop_settimana=not session.imported_plan # Per i futuri test e creazione
        if not (session.date and session.date_end):
           raise osv.except_osv(('Attenzione:'),('Impostare data partenza e fine della edizione!'))

        #   2. Verifica che la data inizio edizione non sia maggiore della data di fine (data di fine obbligatoria da programma)
        inizio_edizione=datetime.strptime(session.date, '%Y-%m-%d %H:%M:%S')  # data iniziale edizione
        fine_edizione=datetime.strptime(session.date_end, '%Y-%m-%d %H:%M:%S')  # data finale edizione
        if inizio_edizione > fine_edizione: # se uguale può andare bene!
           raise osv.except_osv(('Attenzione:'), ('Data inizio edizione maggiore di data fine!'))

        #    3. Controllo se sono state create le holiday
        holiday_list=[] # elenco date escluse
        # Calcolo gli anni tra inizio e fine edizione:
        h_anni_list=[] # TODO tutti gli anni tra i. e f.        
        for anno in range(int(session.date[:4]), int(session.date_end[:4]) + 1):
            if str(anno) not in h_anni_list:
               h_anni_list.append(str(anno))
        # Carico e verifico la presenza delle holiday in questi anni:
        for h_anno in h_anni_list: # verifiche anno da inizio e fine:
            holiday_search=holiday_proxy.search(cr, uid, [('year', '=', h_anno)], context=context)
            if not holiday_search: # se non esistono vanno create prima! (ID anno es: [1])
               raise osv.except_osv(('Attenzione:'),('Prego configurare le ferie (domeniche e festivi)!'))
            else: # se esistono carico la lista delle ferie (serve dopo)
               hp_anno=holiday_period_proxy.search(cr, uid, [('year_id','=',holiday_search[0])], context=context)
               if not hp_anno:
                  raise osv.except_osv(('Attenzione:'),('Trovato almeno un anno senza periodi di ferie!')) 
               for periodo in holiday_period_proxy.read(cr, uid, hp_anno, context=context): # passo tutti i periodi dell'anno
                   hp_start=datetime.strptime(periodo['date_start'], '%Y-%m-%d')  # hanno formato YYYY-MM-GG (senza ore)
                   hp_end=datetime.strptime(periodo['date_stop'], '%Y-%m-%d')
                   hp_attuale=hp_start
                   while hp_attuale <= hp_end: # inserisco tutte le date del periodo nella lista
                         if hp_attuale.isoformat()[:10] not in holiday_list:
                            holiday_list.append(hp_attuale.isoformat()[:10]) # memorizzo la data nel formato ISO
                         hp_attuale=hp_attuale + timedelta(days=1)

        #    4. Controllo se ci sono gli appuntamenti nella settimana tipo
        if not week_proxy.search(cr, uid, [('session_id', '=', session.id)], context=context): # Memorizzare per uso successivo
           raise osv.except_osv(('Attenzione:'), ('Non è stata trovata la settimana tipo da replicare!'))
        #   4. Verifica stato corsi interessati e presenza di tutti i corsi nell'elenco
        #   5. Verifica presenza di tutti gli insegnanti nei corsi interessati 
        #   6. Verifica presenza di tutte le durate dell'elenco corsi/sessione/docente   
        #   7. Verifica che la settimana tipo duri max 6 giorni (al limite controllare che parta di lunedì) 
        elenco_lezioni_wd={0:[],1:[],2:[],3:[],4:[],5:[],6:[],} # elenco lezioni settimana tipo divise per wd
        date_list=[] # elenco date univoco (per test sul numero) << devo averne max 6
        week_ids=week_proxy.search(cr, uid, [('session_id','=',session.id)], context=context)       
        week_read_ids=week_proxy.read(cr, uid, week_ids, context=context)       
        for week_id in week_read_ids: # faccio l'elenco dei giorni pianificati per controllare che siano <=6
            dt_date = datetime.strptime(week_id['date'][:10], '%Y-%m-%d') # solo data
            if week_id['date'][:10] not in date_list:
               date_list.append(week_id['date'][:10]) 
            elenco_lezioni_wd[dt_date.weekday()].append(week_id) # Approfitto per memorizzare le date abbinate al wd
       
        if loop_settimana and len(date_list)>6: # solo nel caso ci sia il loop sett
           raise osv.except_osv(('Attenzione:'),('La settimana tipo ha più di 6 giorni!'))
        
        #   8. TODO Verificare che l'elenco dei corsi sia conforme a quello presente nell'offerta
  
        #  NON BLOCCANTI ###################################################################################

        #   1. Verifico se nell'edizione sono presenti altre lezioni già create (altrimenti le elimino)
        #   if session.seance_ids:            
        #      seance_ids_list=[]
        #      for item in session.seance_ids:
        #          seance_ids_list.append(item.id)   
        #      seance_proxy.unlink(cr, uid, seance_ids_list, context=context)

        #   2. Verifica presenza gruppo dell'edizione altrimenti lo creo
        group_ids = group_proxy.search(cr, uid, [('session_id', '=', session.id)]) # verificare se funziona il browse
        if group_ids:
            group_id = group_ids[0]
        else:
            group_id = group_proxy.create(cr, uid, {'name' : 'Classe: %s' % (session.name,), 'session_id': session.id }, context=context)

        # Inizio programma di creazione lezioni ##############################################################
        seance_id=0 # contatore
        res=[] # elenco che viene ritornato per l'abbinamento all'edizione delle lez. create
        
        if loop_settimana:
           data_attuale=inizio_edizione
           while data_attuale <= fine_edizione: # passo tutte le data dall'inizio alla fine edizione
                 if data_attuale.isoformat()[:10] not in holiday_list: # verifico che non sia festivo per la creazione
                    for lezione in elenco_lezioni_wd[data_attuale.weekday()]:
                        seance_id+=1
                        seance_values = {
                                    'name' : "%s, %s #%d" % (lezione['course_id'][1], session.name, seance_id,), #session.name "Corso + Edizione + %d" % (seance_id,),
                                    'is_first_seance': False,
                                    'session_id': session.id, # non s finale  TODO togliere!!!!!
                                    'session_type': False, # Vedere se adattare
                                    'original_session_id' : session.id, # viene usato il campo già presente (per i related) ??
                                    'course_id' : lezione['course_id'][0],   # Togliere solo i corsi convalidati
                                    'user_id' : session.user_id.id,
                                    'date' : data_attuale.isoformat()[:10] + " " + lezione['date'][11:], # prendo data att + ora lez.
                                    'duration' : lezione['duration'], 
                                    'group_id' : group_id, #fields.many2one('training.group', 'Group', #required=True,help='The group of participants',,
                                    'manual' : session.manual,
                                    'participant_count_manual': session.participant_count_manual,
                                    'presence_form': 'no', #The training center has received the presence list from the lecturer
                                    'base_lecturer_id': lezione['base_lecturer_id'][0],
                                    'base_lecturer_2_id': False, # lezione['base_lecturer_id'][0], TODO non dovrebbe servire se != da scuola!!
                                    'location_id' : session.location_id.id,
                                    }
                        item_created= seance_proxy.create(cr, uid, seance_values, context)
                        if item_created: # se viene creato memorizzo l'id per collegare l'edizione dal chiamante
                           res.append(item_created)
                 data_attuale = data_attuale + timedelta(days=1) # aumento di un giorno la data (end while)
        else: # Riporto pari pari delle lezioni settimanali:
           all_seance_ids=week_proxy.search(cr, uid, [('session_id', '=', session.id)], context=context) # cerco tutte le lezioni della sett. tipo    
           for lezione_item in week_proxy.browse(cr, uid, all_seance_ids, context=context): # browse di tutte le lezioni
               seance_id+=1
               seance_values = {
                                'name' : "%s, %s #%d" % (lezione_item.course_id.name, session.name, seance_id,), #session.name "Corso + Edizione + %d" % (seance_id,),
                                'is_first_seance': False,
                                'session_id': session.id, # non s finale  TODO togliere!!!!!
                                'session_type': False, # Vedere se adattare
                                'original_session_id' : session.id, # viene usato il campo già presente (per i related) ??
                                'course_id' : lezione_item.course_id.id, 
                                'user_id' : session.user_id.id,
                                'date' : lezione_item.date, # TODO CONTROLLARE #data_attuale.isoformat()[:10] + " " + lezione['date'][11:], # prendo data att + ora lez. TODO METTERE LA DATA DEL CALENDARIO
                                'duration' : lezione_item.duration, 
                                'group_id' : group_id, #fields.many2one('training.group', 'Group', #required=True,help='The group of participants',,
                                'manual' : session.manual,
                                'participant_count_manual': session.participant_count_manual,
                                'presence_form': 'no', #The training center has received the presence list from the lecturer
                                'base_lecturer_id': lezione_item.base_lecturer_id.id,
                                'base_lecturer_2_id': lezione_item.base_lecturer_2_id.id or False,
                                'location_id' : session.location_id.id,
                                }
               item_created= seance_proxy.create(cr, uid, seance_values, context)
               if item_created: # se viene creato memorizzo l'id per collegare l'edizione dal chiamante
    ...
    def action_create_seances(self, cr, uid, ids, context=None):
        for session in self.browse(cr, uid, ids, context=context):
            # TODO controllare che non siano già create!
...
class mass_subscription_line(osv.osv_memory):
    ...
    _columns = {
        ...
        'session_id' : fields.many2one('training.session', 'Session',
                                       domain="[('state', 'in', ('draft', 'opened','opened_confirmed', 'closed_confirmed', 'inprogress'))]", required=True),
...
class training_participation(osv.osv):
    ...
    _columns = {
        ...
        'numberhours' : fields.float('ore di assenza', required= False),
...
class training_seance(osv.osv):
    ...
    def _get_sessions_type(self, cr, uid, ids, fieldnames, args, context=None): # Serve ??
        res = []
        for seance in self.browse(cr, uid, ids, context=context):
            types = set()
    ...
    # training.seance
    def name_get(self, cr, uid, ids, context=None): # Da modificare per il calcolo del nome coerente con la visualizzazione a calendario
        return [(obj.id, "%s (%s)" % (obj.name, obj.group_id.name or _('Class %d') % (1,))) for obj in self.browse(cr, uid, list(set(ids)), context=context)]

    def on_change_course(self, cr, uid, ids, course_id, kind, session_id, context=None):
        #import pdb; pdb.set_trace()
        #if not context:
        #     context={}        
        ...
        course = self.pool.get('training.course').browse(cr, uid, course_id, context=context)
        session_course = self.pool.get('training.lecturer.base')

        find_course_id=session_course.search(cr, uid, [('course_id', '=', course_id),('session_id', '=', session_id)], context=context) 
        if find_course_id:
           proxy_session_course=session_course.browse(cr, uid, find_course_id[0],context=context)             
           name= "%s [%s]" % (course.name, proxy_session_course.session_id.name) 
           lecturer_id=proxy_session_course.base_lecturer_id.id
        else:
           name=course.name 
           lecturer_id=False 
        ...
        return {
            'value':{
                'name' : name, ##course.name, #"%s [%s]" % (course.name, course.session_id.name),
                'min_limit' : course.course_type_id.min_limit,
                'max_limit' : course.course_type_id.max_limit,
                'duration' : course.duration,
                'base_lecturer_id': lecturer_id,
            }
        }
    ...
    _columns = {
        ...
        'name' : fields.char('Name', size=64, required=True, select=1),
        'session_ids' : fields.many2many('training.session',   # SOSTITUITO CON SESSION_ID
                                         'training_session_seance_rel',
                                         'seance_id',
                                         'session_id',
                                         'Sessions',
                                         select=1,
                                         ondelete='cascade'),
        'sessions_type': fields.function(_get_sessions_type,  # DA ADATTARE A SESSION_ID
                                         method=True,
                                         string='Session(s) Type',
                                         type='char',
                                         size=32,
                                         ),
        'forced_lecturer' : fields.boolean('Forced Lecturer(s)'),            # Serve??
        'confirmed_lecturer' : fields.function(_confirmed_lecturer_compute,  # DA ELIMINARE
                                               method=True,
                                               select=1,
                                               store={
        ...
        'duplicata' : fields.boolean('Duplicata', required=True),    # Serve ??
        'duplicated' : fields.boolean('Duplicated', required=True),  # Serve ??
...
#                                            domain="[('group_id', '=', group_id)]" #error in v6.0 RC1
        ...
        'contact_ids' : fields.one2many('training.participation.stakeholder', 'seance_id', 'Lecturers', readonly=True), # DA SOSTITUIRE
        'contact_names' : fields.function(_contact_names_compute, method=True,                                          # DA SOSTITUIRE
                                          type='char', size=256,
                                          string='Lecturers'),
        'course_id' : fields.many2one('training.course',   # Usare per il link al modulo
                                      'Course',
                                      select=1,
                                      domain="[('state_course', '=', 'validated')]"),
        'state_course' : fields.related('course_id', 'state_course',
                                        ...
                                        selection=[('draft', 'Draft'),
                                                   ...
                                                   ('validated', 'Validated')],
                                        readonly=True),
        'purchase_line_ids' : fields.one2many('training.seance.purchase_line', 'seance_id', 'Supplier Commands'),  # Da eliminare
        ...
        'shared' : fields.function(_shared_compute,   # FORSE ELIMINARE
                                   method=True,
                                   string='Shared',
                                   type='boolean',
        ...
        'master_id' : fields.many2one('training.seance', 'Master Seance'),  # ELIMINARE ??
    ...
    # training.seance
    def action_workflow_confirm(self, cr, uid, ids, context=None):  # DA CAMBIARE SICURAMENTE #################################
        proxy = self.pool.get('training.participation')
    ...
    # training.seance
    def action_workflow_done(self, cr, uid, ids, context=None):   #  DA CAMBIARE SICURAMENTE #################
        workflow = netsvc.LocalService('workflow')
    ...
    # training.seance
    def action_workflow_cancel(self, cr, uid, ids, context=None): # DA CAMBIARE SICURAMENTE #########################
        workflow = netsvc.LocalService('workflow')
    ...
    # training.seance
    def create_procurements(self, cr, uid, ids, context=None): # DA ELIMINARE ##########################
        purchase_order_pool = self.pool.get('purchase.order')
        location_id = self.pool.get('stock.location').search(cr, uid, [('usage', '=', 'internal')], context=context)[0]
    ...
    def unlink(self, cr, uid, ids, context=None): # DA VARIARE ############################
        for seance in self.browse(cr, uid, ids, context=context):
            if seance.state == 'confirmed':
    ...
    def _get_product(self, cr, uid, ids, context=None):   # SERVE???
        assert len(ids) == 1
...
class training_subscription_line(osv.osv):
    ...
    _columns = {
        ...
        'session_id' : fields.many2one('training.session', 'Session', select=1, required=True,
                                       domain="[('state', 'in', ('draft', 'opened', 'opened_confirmed', 'closed_confirmed', 'inprogress'))]",
...
class training_participation_stakeholder_request(osv.osv):
    ...
    def _amount_to_pay(self, cr, uid, ids, name, args, context=None):
        ...
        for request in self.browse(cr, uid, ids, context=context):
            try:
                #pdb.set_trace()   
                #if request.purchase_order_id: # nik (capire perchè non è presente)
                #   if request.purchase_order_id.invoice_ids.id:   
                invoice_id = request.purchase_order_id.invoice_ids.id # se non esiste genera un AttributeError
                if invoice_id: 
                   request_invoices[invoice_id] = request.id
            except AttributeError:
                # catch: "bool object doesn't have 'x' attribute" errors
                pass
    ...
    _columns = {
        ...
        'session_id': fields.many2one('training.session', 'Session', required=True, ), #select=1, readonly=True
        ...
        'kind' : fields.related('session_id', 'offer_id', 'kind', type='char', string="Offer's Kind"), #readonly=True, select=1
        'job_id': fields.many2one('res.partner.job', 'Contact', required=True, select=1, **WRITABLE_ONLY_IN_DRAFT),
        'email' : fields.char('Email', size=128, **WRITABLE_ONLY_IN_DRAFT), #select=1
        ...
        'purchase_order_id': fields.many2one('purchase.order', 'Purchase Order', readonly=False), # readonly=True
