...
import reportlab
import re
from openerp.reportlab.pdfgen import canvas
...
import base64
from openerp.reportlab.platypus.doctemplate import ActionFlowable
from openerp.tools.safe_eval import safe_eval as eval
from openerp.reportlab.lib.units import inch,cm,mm
from openerp.tools.misc import file_open
from openerp.reportlab.pdfbase import pdfmetrics
from openerp.reportlab.lib.pagesizes import A4, letter
...
class _rml_canvas(object):
    ...
    def _textual(self, node, x=0, y=0):
        ...
        for n in node:
            if n.tag == 'seq':
                from openerp.reportlab.lib.sequencer import getSequencer
                seq = getSequencer()
    ...
    def _image(self, node):
        import urllib
        import urlparse
        from openerp.reportlab.lib.utils import ImageReader
        nfile = node.get('file')
        if not nfile:
            if node.get('name'):
